...
class Item(Base):
    ...
    matches: Mapped[list[ProductMatch]] = relationship(
        ...
        back_populates="item",
        cascade="all, delete-orphan",
        order_by="ProductMatch.position",
    )
...
class ProductMatch(Base):
    """A single hypothesis-vs-actual-product comparison generated by the
    'Подбор по характеристикам' feature.
    """
    ...
    hypothesis_brand: Mapped[str | None] = mapped_column(String(256), nullable=True)
    hypothesis_model: Mapped[str | None] = mapped_column(String(256), nullable=True)
    ...
    found_title: Mapped[str | None] = mapped_column(Text, nullable=True)
    found_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    found_domain: Mapped[str | None] = mapped_column(String(256), nullable=True)
    found_snippet: Mapped[str | None] = mapped_column(Text, nullable=True)
    found_price: Mapped[str | None] = mapped_column(String(128), nullable=True)

    specs_compared: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON, nullable=True)
    ...
    item: Mapped[Item] = relationship("Item", back_populates="matches")
